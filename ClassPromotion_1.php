...
if(isset($_REQUEST['student_prom_data_sender'])){
  ...
  else
  {
    ...
    { 
      ...
      $get_details_query = "SELECT * FROM `student_class_details` WHERE `Student_Details_Id` = ".$_REQUEST['student_details_id'][$i]." AND `School_Id` = ".$_SESSION["SCHOOLID"]." AND `Session_End_Year` = ".$_REQUEST["sesion_end_year"]." ";
      ...
      $class_row = $class_result_set->fetch_assoc();
        ...
        $update_db = $update_query_prepre->bind_param("isii",$_REQUEST['promoted'][$i], $student_remarks, $_REQUEST['student_details_id'][$i], $_SESSION["SCHOOLID"]);
      ...
      if ($_REQUEST['promoted'][$i]=='1') 
      {
        ...
        $ins_new_data_prep->execute();
      }
      // check not promoted
     /* else if ($_REQUEST['promoted'][$i]=='0') 
      {
        // update query if not promoted
        ...
        if(!$update_query_prepre->execute())
        {
          $error_msg = $class_details_query_prep->error;
          $el = new LogMessage();
          $sql = $class_details_query;
      ...
      if ($update_db) 
      {
        echo '<div class="alert alert-success" role="alert">
              <strong>Success</strong> Data Has Been Updaetd
          </div>';
      }
      else
      {
...
if(isset($_REQUEST['update_st_roll']))
{
 ...
 else
 {
  ...
  /************** count section wise data ***************/
  if (isset($_REQUEST['get_count_sec_stud_data'])) {
    // $_REQUEST['class_id]
    $count_q = "SELECT COUNT(scd.Student_Details_Id) AS total_stu, cst.Section
    FROM student_class_details scd, class_section_table cst
    WHERE 
    cst.Class_Id = scd.Class_Sec_Id
    AND scd.Class_Id = ? 
    AND scd.Promoted = 0 
    AND scd.School_Id = ?
    GROUP BY cst.Class_Sec_Id";
    $count_q_prepare = $dbhandle->prepare($count_q);
    $count_q_prepare->bind_param("ii",$_REQUEST['class_id'],$_SESSION["SCHOOLID"]);
    $count_q_prepare->execute();
    $result_set = $count_q_prepare->get_result(); $data = array();
    while($rows = $result_set->fetch_assoc()){
      $data[] = $rows;
    }
    echo json_encode($data);
