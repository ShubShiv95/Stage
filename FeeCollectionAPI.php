...
if ($request_type == 'CollectOtherAmounts') {
    ...
    $OtherFee["Cheque"][1]["ReceptNo"] = '';
    $OtherFee["Cheque"][1]["ChequeNo"] = '';
    $OtherFee["Cheque"][1]["BCharges"] = '';
    ...
    if (!$GetChequeBounceAmountRes) {
        ...
        $el = new LogMessage();
        //$el->write_log_message('Module Name','Error Message','SQL','File','User Name');
        $el->write_log_message('Student Cheque Bounce Charges list: Cheque Bounce charge Fetch Error. ', $error_msg, $sql, __FILE__, $_SESSION['LOGINID']);
        mysqli_rollback($dbhandle);
        $json = array("status" => "Error", "message" => "Database Error: Not able to get cheque bounce charges  information. Please try again later.");
        $json = json_encode($json);
        return $json;
    }
    $ChequeBonChg = 0;
    $counter=1;
    while ($row = $GetChequeBounceAmountRes->fetch_assoc()) {
        $OtherFee["Cheque"][$counter]["ReceptNo"] = $row["Recept_No"];
        $OtherFee["Cheque"][$counter]["ChequeNo"] = $row["Cheque_Number"];
        $OtherFee["Cheque"][$counter]["BCharges"] = $row["Bounce_Charge"];
        $counter++;
        //$ChequeBonChg = $ChequeBonChg + $row["Bounce_Charge"];
    }
...
if ($request_type == 'ViewFeeSummary') {
    ...
    while ($RF_result_row = $RF_result_set->fetch_assoc()) {
        ...
        $fee["School_Fee"][] = array(
            ...
            "Receipt" => $RF_result_row["Recept_No"]
        );
    }
