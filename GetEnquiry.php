...
//echo $query;
/*
$query="select * from admission_enquiry_table where";



if($_REQUEST["enqtype"]!='All'){

    $query=$query . " enquiry_status='" . $_REQUEST["enqtype"] . "' ";
    $condition_found++;
}
    ...
    if($_REQUEST["class"]!='All'){
        ...
        if($condition_found>0){
            ...
            $query=$query . " class_id=" . $_REQUEST["class"] . ' ';
            $condition_found++;
        }  
    }
    ...
    if($_REQUEST["session"]!='All'){
        ...
        if($condition_found>0){
            ...
            $query=$query . " session=" . $_REQUEST["session"] . ' ';
            $condition_found++;
        }  
    }
    ...
    if($_REQUEST["locality"]!='All'){
        ...
        if($condition_found>0){
            ...
            $query=$query . " locality_id=" . $_REQUEST["locality"] . ' ';
            $condition_found++;
        }  
    }
    ...
    if($_REQUEST["searchdate"]=='createdon'){
        ...
        if($condition_found>0){
            $query=$query . " and created_on between str_to_date('" . $_REQUEST["fromdate"] . "','%d/%m/%Y') and str_to_date('" . $_REQUEST["todate"] . "','%d/%m/%Y')";            
        }
        else{
            $query=$query . " created_on between str_to_date('" . $_REQUEST["fromdate"] . "','%d/%m/%Y') and str_to_date('" . $_REQUEST["todate"] . "','%d/%m/%Y')";                      
            $condition_found++;
        }  
    }
...
if($_REQUEST["searchdate"]=='followup'){
        ...
        if($condition_found>0){
            $query=$query . " and followup_date between str_to_date('" . $_REQUEST["fromdate"] . "','%d/%m/%Y') and str_to_date('" . $_REQUEST["todate"] . "','%d/%m/%Y')";            
        }
        else{
            $query=$query . " followup_date between str_to_date('" . $_REQUEST["fromdate"] . "','%d/%m/%Y') and str_to_date('" . $_REQUEST["todate"] . "','%d/%m/%Y')";            
            $condition_found++;
        }  
    }    
        ...
        $query=$query . " and school_id=" . $_SESSION["SCHOOLID"];
