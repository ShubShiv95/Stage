...
if (isset($_REQUEST['get_stud_details_by_name'])) {
  if ($_REQUEST['search_type']=='1') {
    ...
    $student_query = "SELECT smt.*, cmt.Class_Name, cst.Section, scd.Stream,scd.Concession_Id, cmtt.Concession_Name from student_master_table smt, student_class_details scd, class_master_table cmt, class_section_table cst, concession_master_table cmtt WHERE smt.Student_Id = scd.Student_Id AND cmtt.Concession_Id = scd.Concession_Id AND cmt.Class_Id = scd.Class_Id AND cst.Class_Sec_Id = scd.Class_Sec_Id AND scd.Promoted=0 and smt.Enabled = 1 AND smt.Student_Id = ? AND smt.School_Id=?";
    $student_query_prep = $dbhandle->prepare($student_query);
